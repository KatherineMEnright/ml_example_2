...
test_pred <- bind_cols(predictions, clean)

metrics(test_pred, truth = dvote, estimate = .pred_class) %>% 
  gt() %>% 
...
split <- initial_split(clean, prop = 0.01)
training_one_pct  <- training(split)
testing_part <- testing(split)
...
logi_reg_fit <- fit(
  logi_reg_glm,
  formula = dvote ~ gender + party + state + office, 
  data = training_one_pct )

predictions <- logi_reg_fit %>%
  predict(new_data =  training_one_pct )

test_pred <- bind_cols(predictions, training_one_pct)
